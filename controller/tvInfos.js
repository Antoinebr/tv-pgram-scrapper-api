...
const getHoursMinutesFromString = (hourMinute) => {
    ...
    // if hourMinute looks like "1h55min"
    if(hourMinute.includes("h") && hourMinute.includes("min") ){

        hourMinute = hourMinute.replace('min','');
...
const makeEnpoint = (channel, date = null) => {
    ...
    const channelCodes = this.channelCodesFactory();
...
exports.getProgram = async (channel, date = null) => {
    ...
    $('.singleBroadcastCard').each((i, elem) => {
        ...
        const endTime = getEndTime(startTime, getHoursMinutesFromString(duration));

        const singleProgramObject = {
            title,
            hour,
            startTime,
            endTime,
        ...
        if (startTime.getTime() < now() && endTime.getTime() > now()) currently.push(singleProgramObject);

        program.push(singleProgramObject);
 ...
 * @param {string} query 
 */
exports.searchChannel = (query) => {

    const channels = this.channelCodesFactory();
